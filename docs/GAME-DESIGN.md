...
### Classes (`CLASSES` in defs.js)

| Class | dmgMult | lifeMult | Range | Attack cd | Cleave | Skill |
|---|---|---|---|---|---|---|
| Druid | 1.25 | 1.2 | 2.0 | 1.05s | 1.4 | Entangling Burst: r4.2, x3.0 dmg, 2s snare, 60 mana, 5s cd |
| Archer | 0.9 | 1.0 | 7.5 | 0.7s | none | Piercing Shot: 10-long line, x2.6 dmg, 45 mana, 4s cd |
| Mage | 1.05 | 0.85 | 5.5 | 0.85s | 0.9 | Frost Nova: r3.8, x2.2 dmg, 45% slow 3s, 70 mana, 4.5s cd |

Cleave hits secondary targets near the main target for 60% damage.

## Enemies

`enemyStats(tier, rarity)`: hp `220 * 1.32^(tier-1) * hpMult`, damage `(10 + 5*tier) * dmgMult`, speed 3.1 tiles/s, aggro radius 11 tiles, contact bite every 0.9s.
...
Waypoints: entrance + rooms nearest to 45% and 75% of the entrance-to-boss distance. Fog: 9-tile reveal radius; wall segments render only once their floor tile is seen. Enemies render within 14 tiles and only on seen tiles.
